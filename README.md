...
* 采用 `Plan - Act - Log - Verify` 闭环流
* 使用 `log_change.py` 自动记录变更
* 支持模板模式和命令行模式

#### 2 Knowledge Gardener (速记员)
...
* 使用 `batch_processor.py` 批量执行
* 采用软删除机制，确保安全

#### 4 Knowledge Doctor (医生)
