...
| 2 | **读取模板** | - | 读取 `.trae/skills/integrity-check/templates/commit_message.md` |
| 3 | **生成消息** | - | 根据 `git diff` 摘要和 Inbox 笔记内容填充模板，严格遵循 Conventional Commits 规范 |
| 4 | **写入临时文件** | - | 将生成的消息写入 `.git/COMMIT_EDITMSG_TEMP` |
| 5 | **静默提交** | `git commit -F .git/COMMIT_EDITMSG_TEMP` | 使用临时文件中的消息进行提交 |
| 6 | **推送到远程** | `git push origin main` | 将本地提交推送到远程仓库 |
