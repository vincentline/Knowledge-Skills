...
使用 `AskUserQuestion` 工具与用户交互，提供清晰的选项和补充信息输入。
...
任务完成后，**必须**检查是否有值得总结的经验（如：踩过的坑、架构决策、独特的布局方案）。

- **执行方式**: 如果有价值，**不要**询问用户，**直接**调用 `/skill knowledge-gardener` 将其静默写入 Inbox。
- **效果**: 让经验沉淀成为背景行为，不打断用户的业务思考。
