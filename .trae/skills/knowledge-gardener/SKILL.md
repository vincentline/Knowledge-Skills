...
description: 当用户请求记录经验、生成笔记、保存 bug 修复、总结开发心得或需要将信息保存到 Inbox 时，必须立即调用此技能。此技能负责将开发经验快速提取并暂存到 Inbox（海马体）中。
version: 3.1.0
---
...
| 触发关键词 | 示例请求 |
|:---|:---|
| **记录经验** | "把这个记下来"、"记录经验"、"生成笔记"、"保存到 inbox" |
| **Bug 修复** | "有个 bug 修复了"、"记录问题"、"修复了一个 bug" |
| **总结经验** | "总结经验"、"整理一下经验"、"保存经验" |

**判断原则：** 凡是涉及开发经验、Bug 修复、技术问题解决等需要记录到 Inbox 的场景，都必须触发此技能。
...
### 3. 智能静默记录 (Silent Recording)
- 如果是在 `coder` 技能或其他开发流程中触发的经验记录，**不需要**使用 `AskUserQuestion` 询问用户。
- 直接使用 `gardener.py` 将经验写入 Inbox，并在最终回复中用一句话带过：“已将该经验暂存至 Inbox”。
- 这避免了频繁打断用户的心流。
