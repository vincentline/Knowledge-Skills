...
- **架构文档**: 自动检测并创建项目架构文档 (`architecture.md`)。
...
2.  **检查输出**:
    ...
    - 若输出 `NEED_ARCHITECTURE`:
        - 分析项目目录结构 (`LS` 工具)。
        - 阅读关键配置文件 (`package.json`, `vite.config.ts` 等)。
        - 查看核心源码目录结构。
        - 填写 `.trae/rules/core/architecture.md` 文档。
...
| `NEED_ARCHITECTURE` | 需要填写架构文档 | 分析项目结构，填写 architecture.md |
