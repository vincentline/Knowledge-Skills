...
### 2. 知识重构 (Rewriting) - **轻量化原则**
Agent 必须将 Inbox 碎片重写为目标 Markdown 格式。
**核心原则：保持轻量，不要过度结构化。** 除非内容涉及明确的 API，否则不要强行写 TS Interface，使用清晰的 Markdown 列表或段落即可。

- **动作**:
  1. 读取 Inbox 笔记原文。
  2. 提取核心原则、注意事项或代码片段。
  3. 将整理后的 Markdown 写入临时文件 `.trae/temp/structured_[filename].md`。
